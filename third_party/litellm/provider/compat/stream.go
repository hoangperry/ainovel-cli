...
func (s *stream) Next() (litellm.Event, error) {
	...
	s.done = true
	// Vá (ainovel-cli): nhiều proxy OpenAI-compat (vd 9Router) đóng stream bằng EOF
	// sạch mà KHÔNG gửi sentinel [DONE]. Nếu đã nhận được ít nhất một chunk hợp lệ thì
	// coi đây là kết thúc bình thường và phát DoneEvent tổng hợp — để lớp trên finalize
	// tool-call và chốt lượt, thay vì báo lỗi "stream ended before [DONE]". Gate theo
	// finish_reason đã nhận: stream đứt thật giữa chừng (chưa có finish) vẫn báo lỗi đúng.
	if s.finish != "" {
		return litellm.DoneEvent{FinishReason: s.finish, Provider: s.spec.providerName(), Model: s.model}, nil
	}
