...
// WorldStore quản lý dòng thời gian, phục bút, quan hệ nhân vật, thay đổi trạng thái, luật thế giới, luật văn phong, rà soát và bàn giao.
...
// AppendTimelineEvents thêm sự kiện dòng thời gian.
...
// UpdateForeshadow áp dụng hàng loạt thao tác tăng tiến cho phục bút.
...
// UpdateRelationships gộp các thay đổi quan hệ.
...
// AppendStateChanges thêm thay đổi trạng thái nhân vật.
...
// LoadWorldRules đọc luật thế giới.
...
// SaveReview lưu kết quả rà soát.
...
// HasArcReview kiểm tra chương chỉ định (chương cuối cung) đã lưu rà soát scope=arc hay chưa.
// Đọc lỗi thì xử lý như "chưa lưu", để Router nghiêng về việc phái lại thay vì bỏ qua.
func (s *WorldStore) HasArcReview(chapter int) bool {
	rv, err := s.LoadReview(chapter)
	return err == nil && rv != nil && rv.Scope == "arc"
}

// LoadReview đọc kết quả rà soát chương.
...
// LoadLastReview đọc lần rà soát toàn cục gần nhất.
