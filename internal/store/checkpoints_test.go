...
func TestCheckpointStore_EmptyDigestNotIdempotent(t *testing.T) {
	...
	// digest rỗng không tham gia khử trùng idempotent
...
func TestCheckpointStore_RestoreFromDisk(t *testing.T) {
	...
	// mô phỏng khởi động lại: instance mới nạp từ cùng thư mục
	io2 := newIO(dir)
...
func TestCheckpointStore_SeqNotConsumedOnWriteFailure(t *testing.T) {
	...
	// đổi chính file jsonl thành chỉ đọc để lần OpenFile kế tiếp ghi thất bại
