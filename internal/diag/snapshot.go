...
// Snapshot là snapshot chỉ-đọc của toàn bộ artifact trong thư mục output.
// Mọi hàm quy tắc chỉ nhận Snapshot, không truy cập trực tiếp hệ thống tệp.
type Snapshot struct {
	...
	LoadErrors []string // Lỗi tải không phải NotExist, phân biệt "không có dữ liệu" với "đọc lỗi"
}

// Load đọc toàn bộ artifact từ store, dựng snapshot chỉ-đọc.
// File không tồn tại xem là "không có dữ liệu" (trường giữ giá trị zero); lỗi khác ghi vào LoadErrors.
...
// LatestCompleted trả về số chương đã hoàn thành lớn nhất; không có thì trả về 0.
