...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	go func() {
		...
		emit(StageScan, 0, 0, contentlang.Pick("扫描 simulate 语料...", "Đang quét ngữ liệu simulate..."), nil)
		sources, err := scanSources(opts.SourceDir)
		if err != nil {
			emit(StageError, 0, 0, contentlang.Pick("扫描 simulate 目录失败", "Quét thư mục simulate thất bại"), err)
			return
		}
		if len(sources) == 0 {
			emit(StageError, 0, 0, contentlang.Pick("simulate 目录中没有可分析的 .txt/.md/.markdown 文件", "Thư mục simulate không có file .txt/.md/.markdown nào để phân tích"), fmt.Errorf("no simulation sources"))
			return
		}

		existing, err := deps.Store.Simulation.Load()
		if err != nil {
			emit(StageError, 0, len(sources), contentlang.Pick("读取既有画像失败", "Đọc hồ sơ hiện có thất bại"), err)
			return
		}
		pending := pendingSources(existing, sources)
		if len(pending) == 0 {
			emit(StageDone, 0, len(sources), contentlang.Pick("画像已是最新，未发现新增或变更文章", "Hồ sơ đã mới nhất, không phát hiện bài thêm mới hay thay đổi"), nil)
			return
		}
		...
		for i, source := range pending {
			if err := ctx.Err(); err != nil {
				emit(StageError, i, len(pending), contentlang.Pick("用户取消画像分析", "Người dùng hủy phân tích hồ sơ"), err)
				return
			}
			emit(StageAnalyze, i+1, len(pending), fmt.Sprintf(contentlang.Pick("分析仿写语料 %d/%d：%s", "Phân tích ngữ liệu mô phỏng %d/%d: %s"), i+1, len(pending), source.RelativePath), nil)
			report, err := AnalyzeSource(ctx, deps.LLM, deps.Prompts.Source, source)
			if err != nil {
				emit(StageError, i+1, len(pending), contentlang.Pick("语料分析失败", "Phân tích ngữ liệu thất bại"), err)
				return
			}
		...
		emit(StageMerge, len(pending), len(pending), contentlang.Pick("合并仿写画像...", "Đang hợp nhất hồ sơ mô phỏng..."), nil)
		synthesis, err := MergeSynthesis(ctx, deps.LLM, deps.Prompts.Merge, existing, allReports)
		if err != nil {
			emit(StageError, len(pending), len(pending), contentlang.Pick("画像合并失败", "Hợp nhất hồ sơ thất bại"), err)
			return
		}
		...
		if err := deps.Store.Simulation.Save(profile); err != nil {
			emit(StageError, len(pending), len(pending), contentlang.Pick("保存仿写画像失败", "Lưu hồ sơ mô phỏng thất bại"), err)
			return
		}
		emit(StageDone, len(pending), len(pending), fmt.Sprintf(contentlang.Pick("仿写画像已更新：新增/变更 %d 篇，累计 %d 篇", "Đã cập nhật hồ sơ mô phỏng: thêm/đổi %d bài, tổng %d bài"), len(pending), len(profile.Corpus.Sources)), nil)
	}()
	return events, nil
}
