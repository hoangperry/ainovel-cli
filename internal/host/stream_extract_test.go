...
// TestMain ghim locale nội dung về "zh" cho toàn bộ test gói host: các assertion hiện có
// đối chiếu nguyên văn chuỗi zh (header panel, báo cáo ngân sách, lỗi guardExclusive...),
// trong khi mặc định runtime là "vi". Đặt một lần ở đây giữ mọi test xanh.
func TestMain(m *testing.M) {
	contentlang.Set("zh")
	os.Exit(m.Run())
}

// feedAll nạp một lần, trả về output tích lũy.
...
// feedChunked nạp theo lát cắt với số byte chỉ định, kiểm chứng stream và nạp một lần cho kết quả giống nhau.
...
// ── chế độ tổng quát: obj lồng + array ──
...
func TestExtract_FoundationCharacters(t *testing.T) {
	...
	// Render tổng quát: mọi field đều hiển thị, gồm cả aliases / traits trước đây bị whitelist bỏ qua
...
func TestExtract_FoundationLayeredOutline(t *testing.T) {
	...
	// Thụt lề lồng thể hiện phân cấp
...
// ── save_review: chứa object trong array + array số ──
...
// ── edit_chapter: chế độ tổng quát + string nhiều dòng ──
...
// ── tool loại đọc: mật độ thông tin args thấp nhưng header + field then chốt vẫn phải thấy được ──
...
// Lưới đỡ cho args rỗng: khi coordinator gọi novel_context không truyền tham số thì args là {},
// không thể im lặng hoàn toàn, ít nhất phải xuất header cho người dùng cảm nhận có lần gọi.
...
func TestExtract_DraftChapterRawMarkdown(t *testing.T) {
	...
	// Stream trần: không trang trí, không key prefix
...
func TestExtract_DraftChapterIgnoresOtherFields(t *testing.T) {
	// Field ngoài content phải bị bỏ qua âm thầm, không làm ô nhiễm output
...
// ── bất biến khi chia lát stream ──

// Cùng một input chia lát theo 1/3/7/13 byte, output phải giống hệt nạp một lần.
...
// ── kịch bản góc cạnh: array lồng array, lồng sâu ──

func TestExtract_NestedArrays(t *testing.T) {
	// affected_chapters là array int; ở đây đổi thành array lồng array để kiểm chứng
...
// ── chunk cắt giữa ký tự utf-8 nhiều byte ──

func TestExtract_ChunkSplitInUTF8(t *testing.T) {
	// "中" là 3 byte (E4 B8 AD). Đặt kích thước lát cắt thành 1, bảo đảm mỗi byte được nạp riêng.
	in := `{"goal":"中文测试"}`
...
// ── chế độ stream trần: key cùng tên trong obj lồng không nên trúng nhầm ──

func TestExtract_NakedKeyOnlyTopLevel(t *testing.T) {
	// "content" xuất hiện ở hai chỗ: trong object lồng + cấp đỉnh. Chỉ cái ở cấp đỉnh được stream ra.
...
// ── chế độ stream trần: khi content không phải string thì bỏ qua hết ──

func TestExtract_NakedKeyNonStringValue(t *testing.T) {
	// content bị viết nhầm thành object (không nên xảy ra nhưng phải dung thứ)
...
// ── sau khi cấp đỉnh đóng thì Feed nữa không còn sinh output ──
...
// ── array lồng thẳng array (không qua obj) ──
...
// ── number theo sau là dấu cách rồi dấu phân tách ──

func TestExtract_NumberWithTrailingSpace(t *testing.T) {
	// "chapter": 1 ,  ← thừa dấu cách trước và sau số
