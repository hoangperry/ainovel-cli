...
func (e chapterContextEnvelope) apply(result map[string]any) {
	// Hợp nhất chứ không thay thế: đường đi chương của Execute sẽ apply lần lượt hai envelope (seed + buildChapterContext),
	// gán nguyên khối sẽ khiến lần apply thứ hai vứt bỏ nội dung container của seed, các đường dẫn canonical như working_memory.*
	// theo đó mất hiệu lực (con trỏ prômpt chỉ vào hư không, model chỉ có thể dựa vào mirror tầng đỉnh để dung sai mơ hồ).
...
// mergeEnvelopeSection hợp nhất section vào container sẵn có của result[key]; khi container không tồn tại thì gắn trực tiếp.
...
// buildProgressStatus chỉ trả về tóm tắt tiến độ khi Coordinator gọi (không truyền chapter),
// Writer không cần các thông tin này, tránh gây nhiễu việc viết.
...
//
// Tiêm một điểm: writer / editor / architect / coordinator bất kỳ đường nào gọi novel_context
// đều lấy được preferences nhất quán ở working_memory.user_rules. Đường architect vốn không có working_memory,
// hàm này tạo mới theo nhu cầu (chỉ chứa user_rules); ở đường chapter > 0 working_memory đã tồn tại, nhúng trực tiếp.
//
// Dù Bundle rỗng cũng tiêm, giữ trường ổn định, tránh để LLM thấy user_rules=null mà đi vào nhánh ngoại lệ.
//
// Chiến lược tiêm: chỉ cho LLM thấy structured + preferences — hai mục này mới là preferences cần tuân theo khi sáng tác.
// sources / conflicts là thông tin chẩn đoán (tra cứu xung đột người dùng), không vào LLM; do CLI khởi động panel chẩn đoán hiển thị theo nhu cầu.
...
// buildUserDirectives tiêm yêu cầu sáng tác dài hạn của người dùng vào working_memory.user_directives (đường dẫn canonical).
//
// Cùng là tiêm một điểm như buildUserRules: writer / editor / architect / coordinator bất kỳ đường nào
// đều lấy được danh sách nhất quán. Danh sách rỗng cũng tiêm [], giữ trường ổn định (theo tiền lệ user_rules),
// đồng thời để test nhất quán con trỏ prompt phân tích được tự nhiên. Hình dạng mục xem directiveFacts.
...
func (t *ContextTool) prepareChapterContext(chapter int, envelope *chapterContextEnvelope, warn func(string, error)) contextBuildState {
	...
	// Có đang viết lại chương này không: quyết định novel_context có bù sự kiện "chuyên dùng cho viết lại" không.
	isRewrite := progress != nil && slices.Contains(progress.PendingRewrites, chapter)

	// Phơi bày sự kiện draft đã tồn tại hay chưa: để writer khi bị giao lại tự phán định bỏ qua viết lại hay ghi đè.
	// Chỉ phơi bày exists + word_count, không tiêm chính văn (chính văn để writer tự kéo bằng read_chapter theo nhu cầu).
	...
	// Khi viết lại giao "vì sao sửa + sửa chỗ nào" cho writer: lý do đến từ hàng đợi làm lại, phê bình cụ thể đến từ rà soát chương này
	// (selectReviewLessons chỉ triệu hồi chapter-1..chapter-3, vừa khéo bỏ sót chính chương này, writer lại không có tool đọc rà soát).
	// Chính văn không tiêm ở đây — giữ vững giao ước "chính văn kéo bằng read_chapter theo nhu cầu".
	if isRewrite {
...
// buildStyleStats thống kê phong cách cấp toàn sách trên mọi chương đã hoàn thành, tiêm vào episodic_memory.style_stats.
// Cửa sổ rà soát trong cung truyện vốn mù với "tic câu chữ vài chục lần mỗi chương, hình thái cuối chương đồng cấu, lặp lại xuyên chương", chỉ có
// thống kê toàn sách mới phơi bày được — thống kê thuộc code (xác định), phán quyết thuộc LLM (editor ở chiều aesthetic
// chấm điểm theo con số, writer dựa vào đó tự tránh). Khi số chương không đủ thì stylestat trả về nil, không tiêm.
func (t *ContextTool) buildStyleStats(envelope *chapterContextEnvelope, state contextBuildState) {
	...
	for _, ch := range completed {
		// Chương đơn lẻ đọc thất bại thì bỏ qua: thống kê là sự kiện best-effort, không vì thiếu một chương mà từ bỏ tầm nhìn toàn sách
...
// styleStopwords thu thập tên nhân vật và bí danh để lọc khi khai thác cụm từ — tên nhân vật xuất hiện vốn tần suất cao, không phải vấn đề văn phong.
...
func (t *ContextTool) buildChapterEpisodicMemory(envelope *chapterContextEnvelope, state contextBuildState, warn func(string, error)) {
	...
	// Danh bạ nhân vật phụ: triệu hồi nhân vật phụ hoạt động gần đây, để Writer khi đưa lại nhân vật cũ giữ được giọng/định vị nhất quán
	// Không triệu hồi mọi mục (truyện dài sẽ phình ra), chỉ đưa N mục hoạt động gần nhất, sắp xếp giảm dần theo LastSeenChapter
...
func (t *ContextTool) buildArchitectPlanning(envelope *architectContextEnvelope, warn func(string, error)) {
	...
	// completion_signals trình bày tập trung các sự kiện then chốt về "toàn bộ sách có nên kết thúc không",
	// để Architect khi phán quyết complete_book / append_volume thấy ngay mặt đối chiếu.
	// Nếu rải rác trong progress / compass / foreshadow / layered_outline thì dựa vào LLM tự tính dễ sót.
